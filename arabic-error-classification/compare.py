...
def plot_bars(results):
    ...
    metrics = [
        ("test_f1_macro", "Macro F1 (test)", axes[0]),
        ("n_params", "Trainable params", axes[1]),
        ("train_time_seconds", "Training time (s)", axes[2]),
        ("inference_ms_per_example", "Inference (ms/example)", axes[3]),
    ]
...
def markdown_table(results):
    rows = []
    rows.append("| Model | Test Acc | Macro P | Macro R | Macro F1 | Params | Train time (s) | Inference (ms/ex) | Peak GPU mem (MB) |")
    rows.append("|---|---|---|---|---|---|---|---|---|")
    for arch in ARCHS:
        r = results[arch]
        rows.append(
            f"| {arch.upper()} | {r['test_accuracy']:.4f} | {r['test_precision_macro']:.4f} | "
            f"{r['test_recall_macro']:.4f} | {r['test_f1_macro']:.4f} | {r['n_params']:,} | "
            f"{r['train_time_seconds']:.1f} | {r['inference_ms_per_example']:.4f} | "
            f"{r['peak_gpu_memory_mb']:.1f} |"
        )
    return "\n".join(rows)


def per_class_table(results, arch):
    r = results[arch]
    rep = r["per_class_report"]
    rows = ["| Class | Precision | Recall | F1 | Support |", "|---|---|---|---|---|"]
    for label in r["labels"]:
        c = rep[label]
        rows.append(f"| {label} | {c['precision']:.3f} | {c['recall']:.3f} | {c['f1-score']:.3f} | {int(c['support'])} |")
    return "\n".join(rows)


def error_examples_md(results, n=6):
    out = []
    for arch in ARCHS:
        out.append(f"\n{arch.upper()}: sample mistakes ({results[arch]['n_errors']} wrong out of {results[arch]['test_size']} test examples)\n")
        out.append("| Window text | True label | Predicted |")
        out.append("|---|---|---|")
        for e in results[arch]["errors_sample"][:n]:
            out.append(f"| {e['text']} | {e['true']} | {e['pred']} |")
    return "\n".join(out)
...
def main():
    ...
    best_acc = max(ARCHS, key=lambda a: results[a]["test_accuracy"])
    fastest_train = min(ARCHS, key=lambda a: results[a]["train_time_seconds"])
    fastest_inf = min(ARCHS, key=lambda a: results[a]["inference_ms_per_example"])
    most_params = max(ARCHS, key=lambda a: results[a]["n_params"])

    report = f"""# Task 1: RNN vs GRU vs LSTM on Arabic Error-Type Classification
...
the character level, {results['rnn']['vocab_size']} symbols total, more on
...
### Results by class (best model: {best_acc.upper()})

{per_class_table(results, best_acc)}
...
**1. Which model performs best?** {best_acc.upper()}, with a test accuracy
of {results[best_acc]['test_accuracy']:.4f} and macro F1 of
{results[best_acc]['test_f1_macro']:.4f}. GRU is close behind at
{results['gru']['test_accuracy']:.4f}. Plain RNN is far behind both, at
{results['rnn']['test_accuracy']:.4f}, barely above the random-guess
...
and LSTM, GRU trains faster ({results['gru']['train_time_seconds']:.1f}s vs
{results['lstm']['train_time_seconds']:.1f}s for 15 epochs) and runs faster
at inference time ({results['gru']['inference_ms_per_example']:.4f} vs
{results['lstm']['inference_ms_per_example']:.4f} ms per example). That
...
{results[most_params]['n_params']:,}. It has four gates (input, forget,
...
({results['rnn']['n_params']:,} params) and LSTM is about 3.4 times, much
bigger than it would be with a word-level vocabulary, because the
character vocabulary here is tiny ({results['rnn']['vocab_size']} symbols),
...
{abs(results['gru']['test_accuracy']-results['lstm']['test_accuracy']):.4f})
while training faster and using
{results['lstm']['n_params'] - results['gru']['n_params']:,} fewer
...
{results['rnn']['test_accuracy']:.1%} on roughly 80-character sequences,
...
{results['gru']['test_accuracy']:.1%} and {results['lstm']['test_accuracy']:.1%}.
